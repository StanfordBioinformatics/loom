...
class CollapsedDataNodeSerializer(serializers.HyperlinkedModelSerializer):
    ...
    # 'data' is write_only so that HyperlinkedModelSerializer will skip.
    # We render 'data' in a custom way in to_representation
    ...
    def create(self, validated_data):
        ...
        if data is None:
            raise Exception('no data data. Cannot create DataNode')
    ...
    def _extend_all_paths_and_add_data_at_leaves(
        ...
        # node, while 'data' is the representation of all branches and 
        ...
        if not isinstance(data, list):
            if isinstance(data, dict):
                s = DataObjectSerializer(data=data, context=self.context)
...
class DataNodeSerializer(CollapsedDataNodeSerializer):
    ...
    def to_representation(self, instance):
        if not isinstance(instance, models.Model):
            ...
            repr.update({'data': self._data_tree_to_data_struct(instance)})
            return repr
    ...
    def _data_tree_to_data_struct(self, data_node):
        ...
        if data_node.is_leaf():
            ...
            data = [self.BLANK_NODE_VALUE] * data_node.degree
