    ...
    def testCreateWithParallelInputs(self):
        with self.settings(TEST_DISABLE_ASYNC_DELAY=True,
            ...
            run_request_data = {
                'template': workflow_id,
                'inputs': [
                    fixtures.run_requests.run_request_parallel_inputs,
                ]
            }
        ...
        leaf_nodes = data_tree.get_ready_data_nodes([], 0)
        (path0, node0) = leaf_nodes[0]
        (path1, node1) = leaf_nodes[1]
        self.assertEqual(
            fixtures.run_requests.run_request_parallel_inputs['data']['contents'][0],
            node0.data_object.substitution_value)
        self.assertEqual(
            fixtures.run_requests.run_request_parallel_inputs['data']['contents'][1],
            node1.data_object.substitution_value)
        self.assertEqual(
            RunRequestSerializer(rr, context=get_mock_context()).data[
                'template']['uuid'],
            rr.template.uuid)
