...
echo $LOOM_FORCE_MIGRATEDB_ON_START

DEFAULT_LOG_LEVEL=info
DEFAULT_IP=0.0.0.0
DEFAULT_PORT=8000
DEFAULT_GUNICORN_WORKERS_COUNT=10
DEFAULT_FORCE_MIGRATEDB_ON_START=false
...
FORCE_MIGRATEDB_ON_START=${LOOM_FORCE_MIGRATEDB_ON_START:-$DEFAULT_FORCE_MIGRATEDB_ON_START}

for item in true TRUE True t T yes YES Yes y Y
do
    if [ "$FORCE_MIGRATEDB_ON_START" == "$item" ]; then
        FORCE_MIGRATEDB_ON_START=true
        break
    fi
done
...
if [ "$FORCE_MIGRATEDB_ON_START" == "true" ]; then
