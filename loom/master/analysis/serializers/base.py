...
class NoUpdateMixin(object):
    """For models that should not be edited after creation
...
class SubclassValidator(object):
    """Required by SuperclassModelSerializer. Checks that the data
    matches one of the subclasses, and that the data is_valid for
    that subclass serializer.
    """

    def __call__(self, value):
        self.serializer.MatchingSubclassSerializer = self.serializer._select_subclass_serializer_by_data(self.serializer.initial_data)
        self.serializer.MatchingSubclassSerializer(data=self.serializer.initial_data).is_valid(raise_exception=True)
...
class SuperclassModelSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        # This command assumes SubclassValidator already ran and assigned MatchingSubclassSerializer
        serializer = self.MatchingSubclassSerializer(data=self.initial_data, context=self.context)
        
        serializer.is_valid(raise_exception=True) # previously checked if validator was used
        return serializer.save()

    def update(self, instance, validated_data):
        # This command assumes SubclassValidator already ran and assigned MatchingSubclassSerializer
        serializer = self.MatchingSubclassSerializer(instance, data=self.initial_data, context=self.context)
    ...
    def to_representation(self, instance):
        if not isinstance(instance, models.Model):
            ...
            # "instance" is an OrderedDict. It may be missing data in fields that are
            # on the subclass but not on the superclass, so we go back to initial_data.
            SubclassSerializer = self._select_subclass_serializer_by_data(self.initial_data)
            serializer = SubclassSerializer(data=self.initial_data)
            return super(serializer.__class__, serializer).to_representation(self.initial_data)
        else:
            assert isinstance(instance, self.Meta.model)
            # Execute "to_representation" on the correct subclass serializer
            SubclassSerializer = self._select_subclass_serializer_by_model_instance(instance)
            serializer = SubclassSerializer(instance, context=self.context)
            return super(serializer.__class__, serializer).to_representation(instance)
    ...
    def _select_subclass_serializer_by_data(self, data):
        ...
        if len(matching_serializers) == 0:
            raise serializers.ValidationError("Serializer %s does not have any subclasses matching the data given: %s"
                            % (self.__class__.__name__, data))
        elif len(matching_serializers) > 1:
            raise serializers.ValidationError("Multiple subclasses (%s) of serialier %s match the input data: %s"
    ...
    def _select_subclass_serializer_by_model_instance(self, instance, data=None):
        ...
        if len(matching_serializers) == 1:
            ...
            raise serializers.ValidationError("Could not deserialize because the data did not match any subclass")
        else:
            # We expect at most 1 matching subclass, but this is not enforced by Django
            raise serializers.ValidationError("Could not deserialize because the model instance to be updated has multiple "\
